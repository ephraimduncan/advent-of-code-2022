...
const arrSpl = i3b.split("\n").map((sack) => {
  return Array.from(new Set(sack.split(""))).join("");
});

for (let i = 0; i < arrSpl.length; i += 3) {
  const chunk = arrSpl.slice(i, i + 3);
  res.push(chunk);
}

res.forEach((array) => {
  for (let a of array[0]) {
    for (let b of array[1]) {
      for (let c of array[2]) {
        if (a == b && b == c) {
          s3b += csfb(a);
          break;
        }
