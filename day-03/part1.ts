function csf(c: string) {
  return "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(c) + 1;
}

const i3 = `MVWpzTTrTFNNLtssjV
...
const r = i3
  .split("\n")
  .map((elf) => {
    return [elf.slice(0, elf.length / 2), elf.slice(elf.length / 2, elf.length)];
  })
  .map((sack) => {
    return [
      Array.from(new Set(sack[0].split(""))).join(""),
      Array.from(new Set(sack[1].split(""))).join(""),
    ];
  })
  .forEach((sack) => {
    for (let c of sack[0]) {
      for (let sc of sack[1]) {
        if (c === sc) {
          s3 += csf(c);
          break;
        }
